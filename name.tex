...
\title{All-round Antenna Selection for Adaptive Beamforming in MIMO Arrays}
...
\begin{abstract}
Increasing the number of transmit and receive elements in Multiple-Input-Multiple-Output (MIMO) antenna arrays has led to a substantial increase in hardware and computational cost. We mitigate this problem by employing a reconfigurable MIMO array where a large transmit and receive array is multiplexed to a smaller set of $k$ baseband signals. By breaking the MIMO array into four stages, we propose four different selection strategies to offer dimensionality reduction in post-processing, and hardware cost reduction in digital signal processing (DSP) and radio-frequency (RF) stages.
We cast the problem as a determinant maximization and propose a unified formulation to decouple the joint problem and be able to control all four stages in one integrated problem. We then analyze the performances of the proposed selection approaches and prove that that in terms of the output SINR, joint selection outperform the rest of the methods, followed by MFC, hybrid and factored selections. We present numerical results on the performance of the proposed selection methods and show that while all selection modes allow an excellent trade-off between performance and cost, the performances validate the mathematical proofs. 
\end{abstract}
\begin{IEEEkeywords}
